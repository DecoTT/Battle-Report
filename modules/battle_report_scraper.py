...
class InstanceTracker:
    ...
    def should_process(self, hero_name: str, y_center: int) -> bool:
        ...
        - Se salta únicamente si ya hay una instancia *procesada* del mismo
          héroe **con gametag**.
        - Las entradas hero+UNKNOWN nunca bloquean; sirven solo para mergear
          al capturar el gametag.
        - También se evita reabrir cards procesadas muy cerca de la última Y
          procesada para reducir doble click sobre la misma card.
        """
        ...
        for (stored_hero, stored_tag), card in self.seen.items():
            ...
            # Si ya capturamos un gametag para este héroe, bloquea
            if stored_tag is not self.UNKNOWN and card.processed:
                return False

            # Si la card fue marcada procesada cerca del rango superior,
            # evita recapturar la misma card por rebotes de scroll.
