...
def draw_debug_overlay(log_screenshot: np.ndarray, heroes_found: List, captains_found: List) -> np.ndarray:
    ...
    for name, card_type, (abs_x, abs_y), conf in heroes_found:
        ...
        # Rectángulo de la card reducido para un mejor ajuste
        # Se reduce el tamaño a aproximadamente 2/3 del ancho y alto original
        # Ancho original ~90, alto original ~110 → nuevo ancho ~60, alto ~73
        card_w = int(90 * 2/3)
        card_h = int(110 * 2/3)
        cv2.rectangle(debug_img, (rel_x, rel_y), (rel_x + card_w, rel_y + card_h), 
                     COLORS['hero'], 3)
        
        # Punto de click en el centro ajustado (+20, +20 aprox)
        # Ajustamos el offset proporcional al nuevo tamaño del card
        click_x = rel_x + card_w // 3  # ~20 px
        click_y = rel_y + card_h // 3
        cv2.circle(debug_img, (click_x, click_y), 6, COLORS['click_point'], -1)
        cv2.circle(debug_img, (click_x, click_y), 7, COLORS['hero'], 2)
    ...
    for name, card_type, (abs_x, abs_y), conf in captains_found:
        ...
        # Rectángulo de la card reducido para capitanes
        # 2/3 del tamaño original de 70x90 → 46x60 aprox
        cap_w = int(70 * 2/3)
        cap_h = int(90 * 2/3)
        cv2.rectangle(debug_img, (rel_x, rel_y), (rel_x + cap_w, rel_y + cap_h), 
                     COLORS['captain'], 3)
        
        # Punto de click en el centro ajustado (+15, +15 aprox)
        click_x = rel_x + cap_w // 3
        click_y = rel_y + cap_h // 3
        cv2.circle(debug_img, (click_x, click_y), 5, COLORS['click_point'], -1)
        cv2.circle(debug_img, (click_x, click_y), 6, COLORS['captain'], 2)
